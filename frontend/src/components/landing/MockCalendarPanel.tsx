  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 pt-8 pb-3 flex items-center justify-between">
        <h3 className="text-sm font-bold text-atm-ink">{month}</h3>
        <CalendarDays className="w-4 h-4 text-atm-muted" />
      </div>

      <div className="px-4 pb-2">
        <div className="grid grid-cols-7 gap-1 text-[9px] font-mono tracking-wider text-atm-muted text-center mb-1">
          {['일', '월', '화', '수', '목', '금', '토'].map((d) => (
            <span key={d}>{d}</span>
          ))}
        </div>
                ...
                {mark && (
                  <span
                    className={`w-1 h-1 rounded-full mt-0.5 ${
                      mark === 'spent' ? 'bg-atm-ink' : 'bg-atm-accent'
                    }`}
                  />
                )}
              </div>
      ...
      <div className="mt-auto px-3 pb-4">
        <div className="bg-white border border-stone-200 rounded-2xl p-3">
          <div className="text-[9px] font-mono tracking-[0.18em] uppercase text-atm-muted mb-1.5">
            Upcoming
          </div>
          <ul className="space-y-1">
            {upcoming.map((u) => (
              <li
                key={u.day}
                className="flex items-center justify-between text-[11px] text-atm-ink"
              >
                <span className="flex items-center gap-1.5">
                  <span className="w-4 h-4 rounded-full bg-atm-accent/15 text-atm-accent text-[9px] font-mono flex items-center justify-center">
                    {u.day}
                  </span>
                  {u.label}
                </span>
                <span className="font-mono text-atm-muted">{u.amount}</span>
              </li>
            ))}
          </ul>
        </div>
