...
## 7. 브랜드 이니셔티브 R&R (moa365 플레이북 → 서비스 반영)

> 가이드: [`docs/moa365-service-update-plan.md`](../../docs/moa365-service-update-plan.md). 근거: `moa365-brand-playbook.md`.

| 역할 | 담당 범위 | 페이즈 |
|---|---|---|
| R1 Architect | 마스터 가이드·페이즈 분해·플랜 | P0~P2 |
| R3 Frontend | 디자인 토큰·Pretendard·`DotLine`·랜딩 시맨틱 매핑·온보딩·빈화면 / (P2) 연속기록 UI·리뷰 블러 UI·Moa Wrapped | P0·P1·P2 |
| R2 Backend | (P2) streak 엔드포인트·월간리뷰 **서버측 게이팅**·D1/D3 푸시 스케줄러·푸시 i18n | P2 |
| R7 AI Prompt | (P2) `month_insight` 4단 + 유저 문장 **인용 강제**, Wrapped 카피 생성 | P2 |
| R8 Designer/UX Writer | 4색·점선·Pretendard 일관성, **L1/L2/L3 레이어 위반 검수**, 온보딩/스토어/푸시 문안 | P0~P2 |
| R4 QA/Reviewer | **플레이북 부록 체크리스트 게이트**(행동=평어 / 의미=여정만 / 점선 1회 / AI스멜 금지) — 수정 금지, 이슈만 | 전 PR |
| R5 Test/Eval | `month_insight` 골든셋(4단 포맷·인용 정확성), streak 단위테스트 | P2 |
| R9 Security | (P2) 리뷰 블러 **서버측 게이팅** 우회 가능성 검수 | P2 |
| R10 Legal | 스토어 카피·프리미엄 표기·약관 영향(리뷰 유료화) | P1·P2 출시 직전 |
| R6 DevOps | 배포 분리(web vs app), Pretendard 호스팅, 앱 릴리스(v1.4.6) | 각 머지 |

**코드 밖(마케팅 트랙, 엔지니어링 R&R 아님):** 브랜드필름·숏폼 3종·Threads/YouTube/블로그 SEO·#하루한줄 30일 챌린지·VN 트랙 — 플레이북 Part 5·6. 별도 운영.

> 검수(R4·R5)·보안(R9)·법무(R10)는 구현자와 분리. 자체평가 금지(§5 원칙 동일).
