...
async def parse_expense(text: str | None, image_b64: str | None, media_type: str | None) -> list[dict]:
...
async def month_insight(month_key: str, current: dict, previous: dict) -> dict:
    """월별 인사이트 생성."""
...
{{"summary":"한두 문장","praise":"잘한 점","concern":"주의할 점","suggestion":"다음달 행동 제안"}}"""
    ...
    try:
        ...
        text_block = next((b.text for b in resp.content if b.type == "text"), "{}")
        return json.loads(_strip_fence(text_block) or "{}") or fallback
    except Exception as e:
